...
def run_command():
        ...
        # Execute from cache if we have a hit
        script_name = os.path.basename('-'.join(sys.argv[1:]))
        script_hash = hashlib.sha256(hiscript.encode()).hexdigest()
        script_cache_path = os.path.join(CACHE_DIR, f"{script_name}-{script_hash}")
        if HI_REGENERATE == "true" and os.path.exists(script_cache_path):
            os.remove(script_cache_path)
    ...
    ### Output script:"""
        ...
        if response.status_code == 200:
            ...
            output_script = [json.loads(line).get('choices', [{}])[0].get('delta', {}).get('content', "") for line in processed_lines]
            ...
            if HI_EXECUTE == "true":
                ...
                process = subprocess.run(['bash', '-s'] + sys.argv[2:], input=script.encode(), stdout=subprocess.PIPE)
